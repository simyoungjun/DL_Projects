...
def convert(model, vocoder, mel_stat, conversion_wav_paths, DEVICE=None):
	...
	for idx, (src_wav_path, ref_wav_path) in tqdm(enumerate(conversion_wav_paths), total=len(conversion_wav_paths), unit='B', ncols=70, leave=False):
		...
		plt.imshow(ref_mel.T, cmap='viridis', interpolation='nearest')  # Plot the 2D array
...
def main(DEVICE):
	...
	load_checkpoint(args.model_checkpoint_path, model, ckpt_idx=9)
	...
	src_paths = [get_path(dataset_root, "p226_354_mic1.flac"), get_path(dataset_root, "p225_335_mic1.flac")]
	ref_paths = [get_path(dataset_root, "p225_335_mic1.flac"), get_path(dataset_root, "p226_354_mic1.flac")]
