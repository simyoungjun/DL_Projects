...
def load_checkpoint(checkpoint_path, model, optimizer=None, scheduler=None, ckpt_idx=-1):
	...
	if optimizer is not None:
		if not(os.path.exists(checkpoint_path)):
			...
			last_model_path = sorted(glob.glob(get_path(checkpoint_path, '*.pth.tar')))[ckpt_idx]
		...
		# a = get_path(checkpoint_path, '*.pth.tar')
		# last_model_path = sorted(glob.glob(get_path(checkpoint_path, '*.pth.tar')))
		last_model_path = sorted(glob.glob(get_path(checkpoint_path, '*.pth.tar')))[ckpt_idx]
		state = torch.load(last_model_path)
		model.load_state_dict(state['model'])
		global_step = 0
