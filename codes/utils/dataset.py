...
def get_src_and_ref_mels_HP(src_path, ref_path, trim_silence=True, frame_length=1024, hop_length=1024, top_db=10):
	src_mel, ref_mel = None, None
 

	if os.path.isfile(src_path) and os.path.isfile(ref_path):
		src_mel, _, src_H_mel, H_, src_P_mel, P_  = get_mel(src_path, trim_silence=trim_silence, frame_length=frame_length, hop_length=hop_length, top_db=top_db)
		ref_mel, _, ref_H_mel, H_, ref_P_mel, P_ = get_mel(ref_path, trim_silence=trim_silence, frame_length=frame_length, hop_length=hop_length, top_db = top_db)
	else:
		print("[ERROR] No paths exist! Check your filename.: \n\t src_path: {}   ref_path: {}".format(src_path, ref_path))

	return src_mel, ref_mel, src_H_mel, src_P_mel, ref_H_mel, ref_P_mel
